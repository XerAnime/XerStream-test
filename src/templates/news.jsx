...
function News() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await axios.get('https://msks.vercel.app/news/ann/recent-feeds');
				...
				<div className='news'>
					{data && data.map((item, index) => (
					   <Link to={`/news/${encodeURIComponent(item.id)}`} key={index} className='newsBox-n' target='_new'>
							...
							<img src={`https://proxy.anixer42.workers.dev/image-proxy?url=${item.thumbnail}`} alt="Thumbnail" />
						</Link>
					))}
				</div>
			)}
